...
with st.sidebar:
    ...
    monthly_growth_pct = st.number_input(_("monthly_growth_label", "Рост выручки, %"), 0.0, 1000.0, value=5.0, step=0.1)
    churn_pct = st.number_input(_("churn_label", "Отток клиентов, %"), 0.0, 100.0, value=2.0, step=0.1)
    ...
    fixed_costs = st.number_input(_("fixed_costs_label", "Постоянные расходы, ₽"), 0.0, value=3000.0, step=100.0)
    variable_costs_pct = st.number_input(_("variable_costs_label", "Переменные расходы, %"), 0.0, 100.0, value=20.0, step=0.1)
    payment_lag_days = st.number_input(_("payment_lag_label", "Задержка платежей, дней"), 0, 365, value=30, step=1)
    horizon_months = st.number_input(_("horizon_label", "Горизонт, мес."), 1, 120, value=12, step=1)
...
if st.session_state.calc_result:
    ...
    with left:
        ...
        allow_send_ai = st.checkbox(_("ui.ai_consent_checkbox_label", "Разрешаю отправку данных в AI"), value=False, key="allow_send_ai")
    ...
    with right:
        ...
        if selected_tab_key == "pnl":
            ...
            pnl_excel = df_to_excel_bytes(pnl_to_export.rename(columns=headers), index_name)
            cf_excel = df_to_excel_bytes(cf_to_export.rename(columns=headers), index_name)
            ...
            st.download_button(_("ui.download_pnl", "Скачать P&L (Excel)"), pnl_excel, _("ui.pnl_filename", "pnl.xlsx"), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            st.download_button(_("ui.download_cf", "Скачать денежный поток (Excel)"), cf_excel, _("ui.cf_filename", "cashflow.xlsx"), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            
            st.download_button(_("ui.download_inputs", "Скачать сценарий (JSON)"), inputs_json, _("ui.inputs_filename", "scenario.json"), "application/json")
            st.download_button(_("ui.download_scenario_excel", "Скачать сценарий (Excel)"), inputs_excel, "scenario_parameters.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
